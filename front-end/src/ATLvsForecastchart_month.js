...
function getJson_forecasted(arr){
    ...
    for (var i = 0; i < arr.length; i++){
       c.push(arr[i].DayAheadTotalLoadForecastByDayValue);
  }return c;}

function getJson_actual(arr){
    var p=[];
    for (var i = 0; i < arr.length; i++){
      p.push(arr[i].ActualTotalLoadByDayValue);
  }return p;}
...
class ATLvsForecastchart_month extends Component {    constructor(props){
    super(props);
    this.state = {
        chartData: {
            labels:getJson_day(this.showdata()),
            datasets:[
                {
...
static defaultProps = {
            ...
            <Bar
                ...
                options={{
                    ...
                    title:{
                        ...
                        text: 'Actual vs Forecast Total Load in '+this.state.country+' ('+this.state.mapcode+') on '+this.state.month+'/'+this.state.year,
                        fontSize:25
                    },
                    legend:{
 ...
 export default ATLvsForecastchart_month;